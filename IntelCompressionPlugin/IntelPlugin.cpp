...
// REMOVED: Static initializer might be causing DLL load failure
// If DLL doesn't load, static constructors can prevent Photoshop from loading the plugin
...
// TEX format definitions (needed for export)
enum tex_format_export: uint8_t {
	tex_format_export_dxt1 = 0xA,
	tex_format_export_dxt5 = 0xC,
	tex_format_export_bgra8 = 0x14
};
...
{ 
	...
	memset(&texHeader, 0, sizeof(TEX_HEADER_EXPORT));
	texHeader.magic[0] = 'T';
	texHeader.magic[1] = 'E';
	texHeader.magic[2] = 'X';
	texHeader.magic[3] = 0;
...
void IntelPlugin::DoReadStart()
{
	...
	memset(header.get(), 0, sizeof(TEX_HEADER));
	...
	{
		// Check for TEX format: signature should be 0x00584554 ("TEX\0" in little-endian)
		// PaintDotNet plugin uses this format, so we match it for compatibility
		uint32_t texSignature = *reinterpret_cast<uint32_t*>(header->magic);
	...
	// Check if we have a valid TEX header
	bool isTex = false;
